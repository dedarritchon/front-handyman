...
const VOLUME_UNITS: Record<string, { name: string; toLiters: number; symbol: string }> = {
  mL: { name: 'Milliliter', toLiters: 0.001, symbol: 'mL' },
  L: { name: 'Liter', toLiters: 1, symbol: 'L' },
  'fl oz': { name: 'Fluid Ounce', toLiters: 0.0295735, symbol: 'fl oz' },
  cup: { name: 'Cup', toLiters: 0.236588, symbol: 'cup' },
  pt: { name: 'Pint', toLiters: 0.473176, symbol: 'pt' },
  qt: { name: 'Quart', toLiters: 0.946353, symbol: 'qt' },
  gal: { name: 'Gallon', toLiters: 3.78541, symbol: 'gal' },
};
...
function VolumeConverter() {
  ...
  const convertAll = () => {
    ...
    const valueInLiters = val * VOLUME_UNITS[baseUnit].toLiters;
    
    Object.keys(VOLUME_UNITS).forEach((unit) => {
      if (unit !== baseUnit) {
        results[unit] = valueInLiters / VOLUME_UNITS[unit].toLiters;
      }
    });
        ...
        <>
          ...
          <UnitCard $isBase>
            <UnitInfo>
              <UnitIcon>🥤</UnitIcon>
              <UnitDetails>
                <UnitCode>{VOLUME_UNITS[baseUnit].symbol}</UnitCode>
                <UnitName>{VOLUME_UNITS[baseUnit].name}</UnitName>
              </UnitDetails>
            </UnitInfo>
            ...
            {Object.entries(conversions).map(([unit, convertedValue]) => (
              <UnitCard key={unit}>
                <UnitInfo>
                  <UnitIcon>🥤</UnitIcon>
                  <UnitDetails>
                    <UnitCode>{VOLUME_UNITS[unit].symbol}</UnitCode>
                    <UnitName>{VOLUME_UNITS[unit].name}</UnitName>
                  </UnitDetails>
                </UnitInfo>
                <RightSection>
                  <ConvertedValue>
